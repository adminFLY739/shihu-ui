        ...
        <Row @keydown.enter.native="handleSearch">
          <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form" v-show="showSearch">
            ...
            <Form-item label="所属租户" prop="tenantId" v-if="userInfo.isSuper">
              <Select v-model="searchForm.tenantId" placeholder="请选择" clearable style="width: 160px">
                <Option :value="item.id" v-for="item in tenantList" :key="item.id" :name="item.name">{{ item.name }}
                </Option>
              </Select>
            </Form-item>
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              {},
              params.row.tenants.map(item => { return item.name }).toString()
            );
          },
        },
        {
  ...
  methods: {
    ...
    enable(v) {
      ...
      this.$Modal.confirm({
        title: "提示",
        content: "<p>确认该会员通过实名认证吗？</p>",
        onOk: () => {
          API_Member.updateHZNUSHMemberStatus(params).then((res) => {
            if (res.success) {
    ...
    del(v) {
      this.$Modal.confirm({
        title: "提示",
        content: "<p>确认不通过该会员的实名认证吗？</p>",
        onOk: () => {
          API_Member.deleteMemberListData(v.id).then((res) => {
            if (res.success) {
