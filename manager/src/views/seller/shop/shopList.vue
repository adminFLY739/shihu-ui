  ...
  <div class="search">
    <Card>
      <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form" v-show="showSearch">
        ...
        <Form-item label="店铺状态" prop="storeDisable">
...
<!--          <Button @click="add" type="primary" icon="md-add">添加店铺</Button>-->
      ...
      <Row type="flex" justify="center" class="mt_10">
        <Page :current="searchForm.pageNumber" :total="total" :page-size="searchForm.pageSize" @on-change="changePage" @on-page-size-change="changePageSize" :page-size-opts="[10, 20, 50]"
          size="default" show-total show-elevator show-sizer></Page>
      </Row>
    </Card>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 修改店铺
    edit(v) {
      this.$router.push({ path: '/shop-operation', query: { shopId: v.id , tenantId: v.tenantId } });
    },
    // 关闭店铺
    disable(v) {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
          disableShop(v.id,v.tenantId).then((res) => {
            this.$Modal.remove();
            if (res.success) {
    ...
    //查看店铺详细
    detail(row){
      this.$router.push({ name: "shop-detail", query: { id: row.id ,tenantId:row.tenantId} });
    },
    // 审核店铺
    audit(v) {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
          shopAudit(v.id, v.tenantId,0).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getDataList();
    ...
    enable(v) {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: () => {
          enableBrand(v.id,v.tenantId).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getDataList();
