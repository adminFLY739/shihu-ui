...
import {shopDetail, shopAdd, shopEdit, getShopByMemberId, shopAudit, getShopDetailData} from "@/api/shops";
...
export default {
  ...
  data() {
    return {
      ...
      tenantId: this.$route.query.tenantId, // 租户id
  ...
  methods: {
    ...
    // 获取店铺详情
    getShopDetail() {
      getShopDetailData(this.shopId,this.tenantId).then((res) => {
        if (res.success) {
    ...
    audit(operation) {
      ...
      let tenantID
      if (operation === "PASS") {
        shopAudit(id,this.tenantId,0).then((res) => {
        ...
        shopAudit(id,this.tenantId,1).then((res) => {
          this.auditModel = false;
          if (res.success) {
            this.$Message.success("操作成功");
            this.$router.push({ name: "shopAuth" });
