<template>
  ...
  <div class="decorate">
    <div class="decorate-title">
      ...
      <Button style="margin-left: 20px" size="small" ghost v-if="res.type == 'tpl_ad_list' ||
        res.type == 'tpl_activity_list' ||
        res.drawer
        " type="primary" @click="selectStyle()">选择风格</Button>
      <Button style="margin-left: 20px" size="small" ghost v-if="res.type == 'promotions' || res.drawerPromotions"
        type="primary" @click="selectPromotions()">选择促销活动</Button>
    </div>
    ...
    <Drawer title="选择风格" :closable="false" width="400" v-model="styleFlag">
      <div class="drawer">
        <div class="drawer-item" @click="clickDrawer(item, index)" v-for="(item, index) in modelData" :key="index"
          v-if="item.drawer">
          <img src alt />
          <span>{{ item.name }}</span>
        </div>
    ...
    <!-- 右侧显示抽屉 -->
    <Drawer title="选择促销活动(最多只能展示两个活动)" :closable="false" width="400" v-model="promotionsFlag">
      <div class="drawer">
        <div class="drawer-item" @click="clickDrawer(item, index)" v-for="(item, index) in modelData" :key="index"
    ...
    <!-- 卡片集合 -->
    <div class="decorate-list" v-if="(res.type != 'tpl_ad_list' &&
      res.type != 'tpl_activity_list' &&
      res.type != 'promotions') ||
      res.drawer ||
      ...
      <div class="decorate-item" v-for="(item, index) in res.options.list" :key="index">
        <div class="decorate-item-title">
          <div>卡片</div>
          <Icon @click="closeDecorate(index)" v-if="res.close || res.type == 'promotionDetail'" size="20" color="#e1251b"
            type="md-close-circle" />
        </div>

        <div class="decorate-item-box">
          <div class="decorate-border" v-if="item.titleWay" v-for="(title_item, title_index) in item.titleWay"
            :key="title_index">
            ...
            <div class="decorate-view">
              ...
              <div class="decorate-view-link">
                <div v-if="res.options.list[0].listWay.length != 0">
                  ...
                  <div v-if="title_item.___index == bindGoods.___index ||
                    title_item.title == bindGoods.type
                    " v-for="(bindGoods, bindGoodsIndex) in res.options.list[0].listWay" :key="bindGoodsIndex"
                    ...
                    <div class="title-btn">
                      <Button @click="
                        ...
                        " style="margin-right: 10px" size="small">上移</Button>
                    </div>
                  </div>
                </div>
                <!-- 显示绑定分类 -->
              ...
              <div class="decorate-view-btn">
                <Button @click="bindGoodsId(title_item, title_index)" size="small">选择商品</Button>
                <Button @click="bindGoodsCategory(title_index)" size="small" style="margin-top: 20px">选择分类</Button>
              </div>
            </div>
          </div>
          <div class="decorate-view" v-if="res.type == 'carousel'">
            <div class="decorate-view-title">
              <Button @click="slotBanner(res, item, index, 'up')" style="margin-right: 10px"
                size="small">上移</Button><Button @click="slotBanner(res, item, index, 'down')" size="small">下移</Button>
            </div>
          </div>
          <!-- 选择照片 -->
          ...
          <div class="decorate-view" v-if="item.title != void 0 && !res.notTitle && res.type == 'title'">
            <div class="decorate-view-title">文字对齐方式</div>
            <div class="card-box">
              <div class="card" :class="{ active: textAlign == 'left' }" @click="changeTextAlign('left')">
                <img :src="require('@/assets/align-text-left.png')" class="align-text" alt="" />
              </div>
              <div class="card" :class="{ active: textAlign == 'center' }" @click="changeTextAlign('center')">
                <img :src="require('@/assets/align-text-center.png')" class="align-text" alt="" />
              </div>
              <!-- <div
                class="card"
          ...
          <div class="decorate-view" v-if="item.title != void 0 &&
            !res.notTitle &&
            (res.type == 'title' ||
              res.type == 'notice' ||
          ...
          <div class="decorate-view" v-if="item.title != void 0 && !res.notTitle && res.type == 'notice'">
            <div class="decorate-view-title">方向</div>
            <div class="decorate-view">
              <Select style="width: 200px" @on-change="changeDirection($event, item)" v-model="item.direction">
                {{ item.direction }}
          ...
          <!-- 填写小标题 -->
          <div class="decorate-view" v-if="item.title1 != void 0 && !res.notTitle">
          ...
          <div class="decorate-view" v-if="res.type === 'notice' && !res.notTitle">
            <div class="decorate-view-title">公告内容</div>
            <div>
              <div v-for="(t, tindex) in item.title" :key="tindex" class="decorate-notice">
                <Input v-model="t.context" style="width: 200px" />
                <Icon @click="removeNotice(tindex)" size="16" type="md-close-circle" />
              </div>
            </div>
          </div>
          ...
          <div class="decorate-view" v-if="!res.notLink">
            ...
            <!-- 已选链接 -->
            <div v-if="item.url && item.url.length != 0" class="decorate-view-link">
              已选链接：
              <span>
                {{
                  ways.find((e) => {
            ...
            <!-- 选择链接、选择图片按钮 -->
            <div>
              <Button ghost size="small" type="primary" @click="clickLink(item, index, res)">
          ...
          <!-- 链接地址-->
          <div class="decorate-view" v-if="item.url && item.url.url !== undefined && item.url.___type == 'other'">
    ...
    <Button v-if="res.type != 'tpl_ad_list' &&
      res.type != 'tpl_activity_list' &&
      !res.notAdd &&
      res.direction != 'horizontal'
      " type="primary" @click="addDecorate(res.type, res)" ghost>添加</Button>
...
export default {
  ...
  watch: {
    res: {  // 监听名为"res"的prop属性
      handler: (val) => {  // 当prop属性发生变化时，执行的回调函数
        // 这里没有具体的代码逻辑，只是占位符，需要根据实际情况编写代码
      },
      deep: true,  // 深度监听，检测对象内部值的变化
    },
  },
  props: ["res"],  // 接收名为"res"的props属性
  ...
  methods: {
    ...
    selectedGoodsData(val) {
      if (!val) return false;  // 如果val为空，则返回false
      let data = val.map((item) => {  // 遍历val数组
        delete item.selected;  // 删除item对象的selected属性
        delete item.intro;  // 删除item对象的intro属性
        delete item.mobileIntro;  // 删除item对象的mobileIntro属性
        return {
          img: item.thumbnail,
          title: item.goodsName,
          type: this.selectedGoods.title,
          ___index: this.selectedGoods.___index,
          ...item,  // 将item对象的所有属性复制到新对象中
        };
      });
      this.res.options.list[0].listWay.push(...data);  // 将data数组的元素添加到res.options.list[0].listWay数组中
      // 清除已经绑定的分类
      this.res.options.list[0].titleWay[this.goodsSelectedIndex].bindCategory = "";  // 将bindCategory属性设置为空字符串
      this.linkType = "";  // 将linkType属性设置为空字符串
    },
