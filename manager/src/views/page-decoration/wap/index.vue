  ...
  <div class="wrapper">
    <div class="model-view">
      ...
      <div class="model-view-menu">
        <draggable class="model-view-menu-item" :list="modelData" :move="handleMove" v-bind="{
          group: { name: 'model', pull: 'clone', put: false, animation: 150 },
          sort: false,
          ghostClass: 'ghost',
        }">
          <li v-for="(model, index) in modelData" v-if="!model.drawer && !model.drawerPromotions" :key="index"
            class="model-item">
            <img src alt />
      ...
      <div class="model-view-content">
        <div class="content">
          ...
          <draggable class="draggable" group="model" ghostClass="ghost" @add="handleContentlAdd" @end="handleContentlEnd"
            v-model="contentData.list">
            <div class="list" v-for="(element, index) in contentData.list" :key="element.key">
              <component class="component" :class="{ active: selected == index }"
                @click.native="handleComponent(element, index)" :is="templates[element.type]" :res="element.options">
              </component>
              <Icon v-if="selected == index" @click="closeComponent(index)" color="#e1251b" size="25" class="close"
      ...
      <div class="model-config">
        <decorate @handleDrawer="handleDrawer" v-if="decorateData" :res="decorateData"></decorate>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleComponent(val, index) {
      this.selected = index;  // 将选中的索引值赋给变量selected
      this.$set(this, "decorateData", val);  // 使用Vue的$set方法将val赋给decorateData属性，以确保响应性
      console.log("点击楼层装修后的decorateData数据：",this.decorateData);
    },
    ...
    handleDrawer(val) {
      ...
      if (val.promotionsType) {
        ...
        console.log("右侧栏回调后的decorateData数据：",this.decorateData);
      }
    },
