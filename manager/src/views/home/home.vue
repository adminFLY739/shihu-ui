...
export default {
  ...
  methods: {
    ...
    //首页统计数据
    async getHomeData() {
      let res = await homeStatistics({tenantId: this.userInfo.tenantId});
      if (res.success) {
        if (
  ...
  mounted() {
    if(!this.userInfo.isSuper&&this.userInfo.tenantId){
      this.params.tenantId=this.userInfo.tenantId;
      this.orderParams.tenantId=this.userInfo.tenantId;
    }
    this.init();
  },
};
