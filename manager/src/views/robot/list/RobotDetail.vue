    ...
    <Card class="mt_10">
      <Tabs value="point" @on-click="robotOperationChange">
        ...
        <TabPane label="发话题" name="order" style="min-height: 200px">
          <Form ref="form" :model="form" :rules="dicussRuleValidate" :label-width="80">
            <FormItem label="标题" prop="title">
              <Input v-model="form.title" placeholder="标题"></Input>
            </FormItem>
            <FormItem label="内容" prop="content">
              <Input v-model="form.content" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="说些什么叭...(不超过400字)"></Input>
            </FormItem>
            <FormItem label="分类" prop="cut">
        ...
        <TabPane label="点赞&收藏&回复" name="address">
        </TabPane>
...
export default {
  ...
  methods: {
    ...
    handleDicussSubmit() {
      ...
      this.$refs.formValidate.validate((valid) => {
        if (valid) {
          API_Robot.addANewDisscuss(this.formValidate).then((res) => {
            if (res) {
              this.$refs.formValidate.resetFields();
              this.formValidate.media = [];
              this.$Message.success("添加成功");
