...
import {applyPhyStoreFirst} from "@/api/NXCphystroeentry";
export default {
  components: { region },
  props: {
    content: {
  ...
  data () {
    return {
      ...
      rules: { // 验证规则
        employeeNum: [
          { required: true, message: '请填写店铺员工总数' },
          { pattern: RegExp.integer, message: '只能填写正整数' }
        ],
        registeredCapital: [
          { required: true, message: '请填写店铺注册资金' },
          { pattern: RegExp.integer, message: '只能填写正整数' }
        ],
