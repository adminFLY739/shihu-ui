                    ...
                    <div slot="content">
                      ...
                      >
                        <template slot="sn" slot-scope="{ row }">
                          <Select v-model="row.sn"  transfer   @on-change="updateSkuTable(row, 'sn')">
                            <Option v-for="item in shipmentsList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                          </Select>
                        </template>
                        <div
                          v-if="baseInfoForm.goodsType !== 'VIRTUAL_GOODS'"
...
export default {
  ...
  methods: {
    ...
    //获取发货地
    GET_Shipments(){
      API_GOODS.getShipmentsList(this.params).then((res)=>{
        if(res.success){
          this.shipmentsList=res.result.records;
        }
      })
    },
    // 获取当前店铺分类
